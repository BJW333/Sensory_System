...
def _get_backends():
    """
    Select appropriate camera backends for current OS.
    Returns:
        List of cv2 backend constants to try in order.  
    """
...
class CameraManager:
    def __init__(self, camera_index: int = 0) -> None:
        ...
        logging.debug("Camera Index: %d", self.camera_index)
        self.cap: Optional[cv2.VideoCapture] = None
        self.backend = None  #Store the successful backend
        
        _BACKENDS = _get_backends() #get backends for camera manager
        for be in _BACKENDS:
            cap = cv2.VideoCapture(camera_index, be)
            if cap.isOpened():
                self.cap = cap
                self.backend = be  #Remember which backend worked based on var above
    ...
    def _grab_loop(self) -> None:
        """
        Internal loop for grabbing frames from the camera.
        Runs in a separate thread to avoid blocking main thread.
        Args:
            None
        Returns:
        ...
        while self._running:
            ...
            if ok and frame is not None:
                ...
                if consecutive_fail > 30: #1 sec of failures
                    ...
                    self.cap.release()
                    time.sleep(1.0)
                    self.cap.open(self.camera_index)  #Use stored value
                    consecutive_fail = 0
    ...
    def get_frame(self, *, wait: bool = True, timeout: float = 0.5):
        ...
        Args:
            wait (bool): If True, block until a new frame is available or timeout occurs.
            timeout (float): Maximum time to wait for a new frame in seconds.
        Returns:
            A copy of the latest frame as a numpy array, or None if no frame is available
        """
        if wait:
    ...
    def stop(self):
        """
        Stop the camera manager and release resources.
        Args:
            None
        Returns:
            None

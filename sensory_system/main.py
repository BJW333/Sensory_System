...
from idle_detection_combined import idle_detection_loop
from app_usage_monitor import _AppUsageMonitor
from obj_face_recog import obj_person_recog_loop
...
class SensorHub:
    ...
    def start(self) -> "SensorHub":
        # ✅ FIXED: Use loop functions directly with supervisor
        self._spawn(
            "idle_detection",
            idle_detection_loop,
            self.camera,
        ...
        # ✅ FIXED: App monitor - create instance and pass run method
        monitor = _AppUsageMonitor()
        self._spawn("app_usage_monitor", monitor.run)  # ← Remove check_interval
        ...
        while not CONTEXT.get("sensorhub_shutdown"):
            ...
            try:
                while thr.is_alive() and not CONTEXT.get("sensorhub_shutdown"):
                    ...
                    # hang check
                    last_hb = CONTEXT.get(f"{name}_last_beat", 0)
...
if __name__ == "__main__":
    hub = start_sensor_hub(debug=True, console_interval=2.0)

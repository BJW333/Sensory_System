# camera_manager.py – robust threaded frame grabber
# ---------------------------------------------------------------------------
# Key upgrades vs. original:
#   • backend selection fallback (AVFoundation → default) so Linux/Windows still work
#   • explicit FPS attribute; auto-estimate if camera reports 0
#   • exposes `read()` synchronous method as alt to get_frame(wait=False)
#   • graceful shutdown with joinable thread & context-manager support
#   • handles camera disconnect mid-run (attempts reopen)
#   • logging rather than bare RuntimeError
# ---------------------------------------------------------------------------
